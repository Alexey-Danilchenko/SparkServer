...
func (s *Server) BeginFlash(ctx context.Context, deviceID string, job *domain.FlashJob) error {
	client, ok := s.registry.GetClient(deviceID)
	if !ok {
		return ErrDeviceOffline
	}
...
func (s *Server) CompleteFlash(ctx context.Context, deviceID string, job *domain.FlashJob) error {
	client, ok := s.registry.GetClient(deviceID)
	if !ok {
		return ErrDeviceOffline
	}
